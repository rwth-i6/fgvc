...
function IntraClassLoss:updateOutput(input, targets)	
    local targets = targets:view(targets:size(1))
    local B = input:size(1)
    local diff = torch.Tensor(B, input:size(2))
    self.centers = self.centers:float() --TODO
    for i=1, B do
        diff[i]:copy(input[i] - self.centers[targets[i]])
    end	
...
function IntraClassLoss:updateGradInput(input,  targets)
    local targets = targets:view(targets:size(1))
    ...
    if (self.mode == 1) then
        for i=1, B do
            self.gradInput[i]:copy((input[i] - self.centers[targets[i]]))  
        end
    elseif(self.mode == 2) then
        for i=1, B do
            local indices_ti = torch.find(targets, targets[i])
            local count = 1 + #indices_ti
            self.gradInput[i]:copy((input[i] - self.centers[targets[i]]) * (1 - self.alpha / count))
        end
    end
